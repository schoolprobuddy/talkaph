...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwvktcLLcr00Z-QjaQ8Tl4OdhZN_80_HcKKmvL8HIFaVgtuo7LdWAauNcKQGSew3kc-eg/exec";
const PAGE_PASSWORD = "ajhaeY*2oi2";
const SHARED_SECRET = "talkaph-tl-4m8z2q";
...
function RecordButton({ row, onDone }) {
  ...
  return (
    <button onClick={startRecording} style={{
      ...
      background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.5)",
      color: "#3b82f6", cursor: "pointer", fontSize: "1.2rem",
    }} title="Tap to record">
      🎙️
    </button>
  );
...
function PasswordGate({ onUnlock }) {
  ...
  function submit() {
    if (input === PAGE_PASSWORD) {
      sessionStorage.setItem("talkaph_record_tagalog_unlocked", "yes");
      onUnlock();
    } else {
      setError(true);
    }
      ...
      {rows.map((row) => {
        ...
        return (
          <div key={row.rowNumber} style={{
            background: recorded ? "rgba(59,130,246,0.07)" : "rgba(255,255,255,0.04)",
            border: recorded ? "1px solid rgba(59,130,246,0.25)" : "1px solid rgba(255,255,255,0.08)",
            borderRadius: "0.6rem",
...
export default function RecordTagalogPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("talkaph_record_tagalog_unlocked") === "yes") {
      setUnlocked(true);
    }
    setChecked(true);
  }, []);
  ...
  async function addPhrase() {
    if (!newPhrase.trim()) {
      alert("Please enter the Tagalog phrase.");
      return;
    }
    setAdding(true);
    try {
  ...
  const tabStyle = (active) => ({
    ...
    borderRadius: "999px",
    border: active ? "1px solid rgba(59,130,246,0.6)" : "1px solid rgba(255,255,255,0.15)",
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            ...
            <div style={{
              ...
              <p style={{ margin: "0 0 0.6rem", fontSize: "0.85rem", fontWeight: "700", color: "#3b82f6" }}>
                + Add a new phrase
              </p>
              <input
                type="text"
                placeholder="Tagalog phrase"
                value={newPhrase}
