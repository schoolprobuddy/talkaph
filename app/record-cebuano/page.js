...
function RecordButton({ row, onDone }) {
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          {!previewUrl && row["Audio URL"] && (
            <button onClick={() => setShowPlayer(!showPlayer)} style={{
              ...
              color: "#10b981", cursor: "pointer", fontSize: "0.75rem", fontWeight: "700",
            }}>
  ...
  return (
    <button onClick={startRecording} style={{
      ...
      background: "rgba(16,185,129,0.15)", border: "1px solid rgba(16,185,129,0.5)",
      color: "#10b981", cursor: "pointer", fontSize: "1.2rem",
    }} title="Tap to record">
      🎙️
    </button>
  );
...
function PasswordGate({ onUnlock }) {
  ...
  function submit() {
    if (input === PAGE_PASSWORD) {
      sessionStorage.setItem("talkaph_record_cebuano_unlocked", "yes");
      onUnlock();
    } else {
      setError(true);
    }
...
export default function RecordCebuanoPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("talkaph_record_cebuano_unlocked") === "yes") {
      setUnlocked(true);
    }
    setChecked(true);
  }, []);
  ...
  async function addPhrase() {
    if (!newPhrase.trim()) {
      alert("Please enter the Cebuano phrase.");
      return;
    }
    setAdding(true);
    try {
  ...
  const tabStyle = (active) => ({
    ...
    borderRadius: "999px",
    border: active ? "1px solid rgba(16,185,129,0.6)" : "1px solid rgba(255,255,255,0.15)",
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
            ...
            <div style={{
              ...
              <p style={{ margin: "0 0 0.6rem", fontSize: "0.85rem", fontWeight: "700", color: "#10b981" }}>
                + Add a new phrase
              </p>
              <input
                type="text"
                placeholder="Cebuano phrase"
                value={newPhrase}
